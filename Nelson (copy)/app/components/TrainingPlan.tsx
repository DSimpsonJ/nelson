...
/* ========= Date helpers ========= */
function getISOWeekId(date = new Date(), weekStart: "monday" | "sunday" = "monday") {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
  const dow = d.getUTCDay() || 7; // Mon=1..Sun=7
  if (weekStart === "monday") {
    d.setUTCDate(d.getUTCDate() + 4 - dow);
  } else {
    const adj = dow === 7 ? 1 : dow + 1;
    d.setUTCDate(d.getUTCDate() + 4 - adj);
  }
...
function playRestSound() {
    const audio = new Audio("/rest-complete.mp3");
    audio.play().catch(() => {});
  }
...
useEffect(() => {
  ...
  async function loadWeekCompletion(weekStart: "monday" | "sunday" = "monday") {
    ...
    const weekId = getISOWeekId(new Date(), weekStart);
  ...
  async function setCompleted(focus: string, dayIndex: number, done: boolean) {
    ...
    const currentWeek = getISOWeekId(new Date(), weekStart);
    const { db } = await import("../firebase/config");
  ...
  async function openDay(focus: string, index: number) {
    ...
    if (email) {
      ...
      setWorkouts(prev => ({ ...prev, [focus]: exerciseLibrary[focus] || [] }));
    }
  ...
  async function commitSwap(option: Exercise) {
    if (selectedDay == null || selectedDayIndex == null || swapIndex == null) {
    ...
    // keep any sets the user already logged for that slot
    const prevSets = current[swapIndex]?.sets;
    updated[swapIndex] = { ...option, sets: prevSets };
    ...
    if (email) {
      ...
      const { doc, setDoc } = await import("firebase/firestore");
      const ref = doc(db, "users", email, "workouts", focus);
      await setDoc(ref, { type: focus, exercises: updated }, { merge: true });
    }

    setWorkouts(prev => ({ ...prev, [focus]: updated }));
    setShowSwapModal(false);
    setSwapIndex(null);
  }
    ...
    <section className="bg-white p-8 rounded-2xl shadow mb-8 relative">
      ...
      {selectedDay && selectedDayIndex !== null && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 rounded-2xl z-20">
          <div className="bg-white text-gray-800 p-6 rounded-xl shadow-xl max-w-sm w-full">
  ...
  playRestSound();
  vibratePhone();
  return <span className="ml-1 text-green-600">Ready ✅</span>;
})()}
                                  </span>
                                );
                              })()}
            ...
            <button
              onClick={async () => {
                ...
                const dayKey = `day-${selectedDayIndex + 1}`;
                const isDone = completion?.byDay?.[dayKey] || false;
                await setCompleted(selectedDay, selectedDayIndex, !isDone);
              }}
            ...
            >
              {(() => {
                const dayKey = `day-${selectedDayIndex + 1}`;
                const isDone = completion?.byDay?.[dayKey] || false;
                return isDone ? "Mark Incomplete ↩︎" : "Mark Complete ✅";
              })()}
            </button>
              ...
              <button
                onClick={async () => {
                  ...
                  const newEx: Exercise = {
                    ...
                    difficulty: "Beginner",
                    sets: Array.from({ length: volume.sets }, () => ({ reps: null, score: null, completed: false })),
                  };

                  const updated = [...(workouts[selectedDay] || []), newEx];
