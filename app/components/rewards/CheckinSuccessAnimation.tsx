...
export default function CheckinSuccessAnimation({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    // Load reward from session storage
    const pendingReward = sessionStorage.getItem('pendingReward');
    if (pendingReward) {
      try {
        const parsed: RewardResult = JSON.parse(pendingReward);
        setReward(parsed.payload);
        sessionStorage.removeItem('pendingReward');
      } catch (error) {
        console.error('[CheckinSuccess] Failed to parse reward:', error);
      }
    }
  }, []);
