...
users/{email}
  ...
  - hasSeenNotificationPrompt: boolean  ← set true when prompt is answered (enable or skip)
  - notificationsEnabled: boolean       ← user preference, drives local notification scheduling
  - notificationHour: number            ← hour of day for daily reminder (0-23)
  - notificationMinute: number          ← minute of hour for daily reminder (0-59)
  - trialStartDate: string              ← ISO string, written on first check-in via /api/submit-checkin
  - isSubscriber: boolean               ← written on validated IAP receipt (Phase 4 Week 1)
...
| Gap reconciliation on mobile | Built and committed March 18, 2026. Mirrors web app behavior: single-day gap only, shown before check-in flow, Yes preserves momentum, No applies 0.92 decay. End-to-end tested and confirmed March 19, 2026. |
...
| route.ts divergence risk | RESOLVED March 18, 2026. Momentum unification complete — all writes via /api/submit-checkin. writeDailyMomentum.ts is dead code (retained as reference, not called). |
...
| Notification infrastructure | Built March 20, 2026. expo-notifications installed. Local daily notifications scheduled on user-selected time. 7 rotating messages (weekly schedule). NotificationPrompt component shows on first dashboard load (hasSeenNotificationPrompt guard). Settings toggle wires to schedule/cancel. APNs (remote push) deferred until Apple Developer enrollment. Notification suppression post-check-in deferred — requires APNs server-side push to implement cleanly. |
| Check-in swipe animation | Built March 19, 2026. Direction-aware slide animation (220ms), tap highlight with 40% dim on unselected, 350ms pause before advance, right swipe > 60px triggers back navigation via GestureDetector. GestureHandlerRootView added to app/_layout.tsx. |
| Gap-fill decay fix | Committed March 19, 2026. Both checkin.tsx and index.tsx gap-fill loops now write decayedMomentum (0.92/day) instead of frozen startingMomentum. Pre-fix docs (March 15-16) are baked in — not correctable retroactively. |
| Paywall screen | Built March 20, 2026. app/(paywall)/index.tsx. Hard gate — no dismiss, gestureEnabled: false. Subscribe and Restore buttons are placeholders until StoreKit is wired in Phase 4 Week 1. |
| Trial gate in _layout.tsx | Built March 20, 2026. Reads isSubscriber and trialStartDate from users/{email}. 14-day trial from first check-in. trialStartDate written by /api/submit-checkin on first check-in (isFirstCheckin guard). Existing alpha users have no trialStartDate — must be backfilled in Firestore before TestFlight invites go out. |
